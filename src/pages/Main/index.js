...
export default function Main({ history }) {
  ...
  React.useEffect(() => {
    if (error) {
      logout();
      toast('You have been logged out for security reasons! Login again');
      history.push('/login');
    }
  }, [error, history]);
      ...
      <Aside>
        ...
        <ContainerFollows>
          ...
          {follows.length === 0 && loading === false && (
            <EmptyMessage message="You don't follow anyone, start following your friends" />
          )}

          {loading && <Spinner />}
        </ContainerFollows>
        <ContainerFooter>
          Information - HelpPress - API - Employment - Privacy - Conditions -
          Directory - Profiles - Hashtags - Language
        </ContainerFooter>
      </Aside>
